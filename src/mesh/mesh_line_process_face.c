...
/*   Updated: 2019/02/26 15:49:11 by fmessina         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool		mesh_line_process_face(t_mesh *mesh, char *str)
{
	...
	if (mesh && str)
	{
		...
		//i = (mesh->n_face[1]++) * mesh->n_face[2] * mesh->n_face[4]; // was like this before but not in the other line processing, why? it's the same you duumbass!
		i = (++mesh->n_face[1] - 1) * mesh->n_face[2] * mesh->n_face[4];
		if (!(split = ft_strsplit(str, ' ')))
			return (error_bool("[ERROR mesh_line_process_face()]\t" \
			"Mesh could not split face line for processing its data!\n"));
		if (!(mesh_process_face(mesh, split, i)))
		{
			split_destroy(split);
			return (error_bool("[ERROR mesh_line_process_face()]\t" \
